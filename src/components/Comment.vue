<template>
  <div class="media g-mb-20">
    <router-link :to="{ path: `/user/${comment.author.id}/` }" :title="comment.author.name || comment.author.username">
      <img class="d-flex g-width-50 g-height-50 rounded-circle g-mt-3 g-mr-20"
           :src="comment.author.avatar"
           :alt="comment.author.name || comment.author.username">
    </router-link>
    <div :class="leftBrdColor" class="media-body g-brd-around g-brd-gray-light-v4 g-brd-left-1 g-pa-20">
      <div class="d-sm-flex justify-content-sm-between align-items-sm-center g-mb-15 g-mb-10--sm">
        <h5 v-if="comment.author.id == comment.post.author" class="h4 g-font-weight-300 g-mr-10 g-mb-5 g-mb-0--sm g-color-pink"><span class="g-mr-5">{{ comment.author.name || comment.author.username }}</span> <button class="btn btn-xs u-btn-inset u-btn-outline-red g-mr-5">博文作者</button></h5>
      ...
      <div class="d-flex justify-content-start">
        <ul class="list-inline mb-0">
          ...
          <!-- 该评论发表在哪篇博客文章下面 -->
          <li class="list-inline-item g-mr-20">
            <router-link :to="{ name: 'PostDetail', params: { id: comment.post.id } }"
        ...
        <ul class="list-inline mb-0 ml-auto">
          <li v-if="comment.author.id == sharedState.user_id" class="list-inline-item g-mr-5">
            <button @click="$emit('edit-comment')" class="btn btn-xs u-btn-outline-purple" data-toggle="modal" data-target="#editCommentModal">编辑</button>
          </li>
          <li v-if="comment.author.id == sharedState.user_id" class="list-inline-item">
            <button @click="$emit('delete-comment')" class="btn btn-xs u-btn-outline-red">删除</button>
          </li>
        </ul>
      </div>
    </div>
