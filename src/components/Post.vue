<template>
  <div :class="leftBrdColor" class="media g-brd-around g-brd-gray-light-v4 g-brd-left-1 g-pa-20 g-mb-20">
   <el-row  class="sharelistBox ">
            ...
            <header>
             ...
             <router-link :to="{ name: 'PostDetail', params: { id: post.id }}" class="g-text-underline--none--hover">{{ post.title }}</router-link>

                </h1>
<h2>
                 <span>  <router-link :to="{ path: `/user/${post.author.id}/` }" :title="post.author.name || post.author.username">
      <img class="g-width-40 g-height-40 rounded-circle rounded" :src="post.author.avatar" :alt="post.author.name || post.author.username">

    </router-link></span> <router-link :to="{ path: `/user/${post.author.id}/` }"><span v-if="post.author.name">{{ post.author.name }}</span><span v-else>{{ post.author.username }}</span></router-link>
...
<!--                </div>-->
        ...
        v-highlight>
            ...
            <router-link :to="{ name: 'PostDetail', params: { id: post.id }}" class="tcolors-bg">阅读全文</router-link>

          <li v-if="post.author.id == sharedState.user_id" class="list-inline-item g-mr-5">
            <!--<button @click="$emit('edit-post')" class="btn btn-xs u-btn-outline-purple" data-toggle="modal" data-target="#updatePostModal">编辑</button>-->

            <button @click="$emit('edit-post')" class="btn btn-xs u-btn-outline-purple" data-toggle="modal" data-target="#editPostModal">编辑</button>
          </li>
          <li v-if="post.author.id == sharedState.user_id" class="list-inline-item">
            <button @click="$emit('delete-post')" class="btn btn-xs u-btn-outline-red">删除</button>
          </li>
                </ul>
            </div>
...
<!--      &lt;!&ndash; vue-markdown 开始解析markdown，它是子组件，通过 props 给它传值即可-->
<!--      v-highlight 是自定义指令，用 highlight.js 语法高亮 &ndash;&gt;-->
<!--      <vue-markdown-->
<!--        :source="post.summary"-->
<!--        class="markdown-body g-mb-15"-->
<!--        v-highlight>-->
...
<!--           <li class="list-inline-item g-mr-20">-->
<!--&lt;!&ndash;             todo path: `/post/${post.id}#comment-list-wrap`&ndash;&gt;-->
<!--            <router-link :to="{ path: `/post/${post.id}#comment-list-wrap` }" class="g-color-gray-dark-v5 g-text-underline&#45;&#45;none&#45;&#45;hover">-->
...
<!--            <router-link :to="{ name: 'PostDetail', params: { id: post.id }}" class="btn btn-xs u-btn-outline-primary">阅读全文</router-link>-->
<!--          </li>-->
<!--          <li v-if="post.author.id == sharedState.user_id" class="list-inline-item g-mr-5">-->
<!--            &lt;!&ndash;<button @click="$emit('edit-post')" class="btn btn-xs u-btn-outline-purple" data-toggle="modal" data-target="#updatePostModal">编辑</button>&ndash;&gt;-->
