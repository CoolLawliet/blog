        ...
        <el-row class="sharelistBox ">
            ...
            <span class="s-round-date">
                ...
                <header>
                    ...
                    <h2>
                 <span>  <router-link :to="{ path: `/user/${post.author.id}/` }"
                                      title="post.author.name || post.author.username">
      <img class="g-width-40 g-height-40 rounded-circle rounded"
           :src="post.author.avatar"
           :alt="post.author.name || post.author.username">

    </router-link></span>
                        <router-link :to="{ path: `/user/${post.author.id}/` }"><span v-if="post.author.name">{{ post.author.name }}</span><span
                                v-else>{{ post.author.username }}</span></router-link>
                        ...
                        <span class="rateBox">
                        <router-link :to="{ path: `/post/${post.id}#like-post` }"
              ...
              <i class="icon-heart g-pos-rel g-top-1 g-mr-3"></i> {{ post.likers_count }} 次喜欢 •
                        ...
                        <router-link :to="{ path: `/post/${post.id}#comment-list-wrap` }"
                                     class="g-color-gray-dark-v5 g-text-underline--none--hover">
              <i class="icon-bubble g-pos-rel g-top-1 g-mr-3"></i> {{ post.comments_count }} 次评论
                    ...
                    <div class="ui label" v-if="catclass">
                        <router-link :to="{ path: `/Share/?classId=${post.category}` }">
                            {{catclass[post.category-1].name}}
                        </router-link>
                ...
                <div class="viewdetail">
                    <ul>
                        ...
                        <router-link :to="{ name: 'PostDetail', params: { id: post.id }}" class="tcolors-bg">
                            阅读全文
                        </router-link>
                        ...
                        <li v-if="post.author.id == sharedState.user_id" class="list-inline-item g-mr-5">
                            ...
                            <button @click="$emit('edit-post')" class="btn btn-xs u-btn-outline-purple"
