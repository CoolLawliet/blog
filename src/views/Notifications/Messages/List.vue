  ...
  <div>
    ...
    <div class="card border-0 g-mb-15">
      ...
      <div class="card-header d-flex align-items-center justify-content-between g-bg-gray-light-v5 border-0 g-mb-15">
        <h3 class="h6 mb-0">
          <i class="icon-bubbles g-pos-rel g-top-1 g-mr-5"></i> 收到的私信 <small v-if="messages">(共 {{ count }} 条, {{page_total }} 页)</small>
        </h3>
...
export default {
  ...
  methods: {
    getUserMessagesSenders (id) {
      ...
      const path = `/api/users/${id}/messages-senders/?page=${this.page}&per_page=${this.per_page}`
      this.$axios.get(path)
        .then((response) => {
          // handle success
