...
<!--           <img v-if="user.avatar" class="img-fluid w-100" :src="user.avatar" :alt="user.name || user.username">-->
            ...
            <button v-if="!is_following && $route.params.id != sharedState.user_id" @click="onFollowUser($route.params.id)" class="btn btn-block u-btn-outline-primary g-rounded-50 g-py-12 g-mb-10">
              <i class="icon-user-follow g-pos-rel g-top-1 g-mr-5"></i> 关注
            </button>

            <button v-if="is_following && $route.params.id != sharedState.user_id" @click="onUnfollowUser($route.params.id)" class="btn btn-block u-btn-outline-red g-rounded-50 g-py-12 g-mb-10">
              <i class="icon-user-unfollow g-pos-rel g-top-1 g-mr-5"></i> 取消关注
            </button>


            <router-link v-if="$route.params.id != sharedState.user_id" :to="{ name: 'MessagesHistoryResource', query: { from: $route.params.id } }" class="btn btn-block u-btn-outline-purple g-rounded-50 g-py-12 g-mb-10">
            ...
            <router-link v-if="$route.params.id == sharedState.user_id" :to="{ name: 'SettingProfile' }" class="btn btn-block u-btn-outline-primary g-rounded-50 g-py-12 g-mb-10">
          ...
          <div class="col-sm-9">
            ...
            <ul class="nav nav-tabs g-mb-20">
              <li class="nav-item">
                <router-link :to="{ name: 'UserOverview' }" :active-class="'active'" class="nav-link" :class="isUserOverView">我的信息 <span class="u-label g-font-size-11 g-bg-primary g-rounded-20 g-px-10"><i class="icon-layers g-pos-rel g-top-1 g-mr-8"></i></span></router-link>
              </li>


              <li class="nav-item">
                <router-link :to="{ name: 'UserFollowers' }" :active-class="'active'" class="nav-link">粉丝 <span class="u-label g-font-size-11 g-bg-deeporange g-rounded-20 g-px-10">{{ user.followers_count }}</span></router-link>
              </li>
              <li class="nav-item">
                <router-link :to="{ name: 'UserFollowing' }" :active-class="'active'" class="nav-link">关注 <span class="u-label g-font-size-11 g-bg-aqua g-rounded-20 g-px-10">{{ user.followeds_count }}</span></router-link>
              </li>
              <li class="nav-item">
                <router-link :to="{ name: 'UserPosts' }" :active-class="'active'" class="nav-link" :class="{'active': $route.name == 'UserFollowingPosts'}">文章 <span class="u-label g-font-size-11 g-bg-purple g-rounded-20 g-px-10">{{ user.posts_count }}</span></router-link>
              </li>
