                                ...
                                <form @submit.prevent="onSubmitUpdate" @reset.prevent="onResetUpdate">
                                    <div class="form-group" :class="{'u-has-error-v1': editForm.titleError}">
                                        <input type="text" v-model="editForm.title" class="form-control"
                                               id="editform_title" placeholder="标题">
                                        <small class="form-control-feedback" v-show="editForm.titleError">{{
                                            editForm.titleError }}
                                        </small>
                                    </div>
                                    <div class="form-group">
                                        <input type="text" v-model="editForm.summary" class="form-control"
                                               id="editform_summary" placeholder="摘要">
                                        <small class="form-control-feedback" v-show="editForm.summaryError">{{
                                            editForm.summaryError }}
                                        </small>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <small class="form-control-feedback" v-show="editForm.categoryError">{{
                                            editForm.categoryError }}
                                        </small>
                                    ...
                                    <div class="form-group">
                                        <textarea v-model="editForm.body" class="form-control" id="editPostFormBody"
                                                  rows="5" placeholder=" 内容"></textarea>
                                        <small class="form-control-feedback" v-show="editForm.bodyError">{{
                                            editForm.bodyError }}
                                        </small>
                                    </div>
                ...
                <div class="modal fade" id="editCommentModal" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="editCommentForm" @submit.prevent="onSubmitUpdateComment"
                                      @reset.prevent="onResetUpdateComment">
                                    <div class="form-group">
                                        <textarea v-model="editCommentForm.body" class="form-control"
                                                  id="editCommentFormBody" rows="5" placeholder=" 评论内容"></textarea>
                                        <small class="form-control-feedback" v-show="editCommentForm.bodyError">{{
                                            editCommentForm.bodyError }}
                                        </small>
                                    </div>
                                    <button type="reset" class="btn btn-secondary">Cancel</button>
                                    <button type="submit" class="btn btn-primary">Update</button>
                        ...
                        <article class="g-mb-60 g-pt-15 g-pb-50">
                            <header class="g-mb-30">
                                ...
                                <ul class="list-inline d-sm-flex g-color-gray-dark-v4 mb-0">
                                    <li v-if="post.author && post.author.id === sharedState.user_id"
                                        class="list-inline-item">
                                        <button @click="onEditPost(post)"
                                                class="btn btn-xs u-btn-outline-purple g-mr-5" data-toggle="modal"
                                                data-target="#updatePostModal">编辑
                                        </button>
                                    </li>
                                    <li v-if="post.author && post.author.id === sharedState.user_id"
                                        class="list-inline-item">
                                        <button @click="onDeletePost(post)"
                                                class="btn btn-xs u-btn-outline-red g-mr-5">删除
                                        </button>
                                    </li>
                                    <li class="list-inline-item">
                                        <a href="#comment-list-wrap"
                                           class="btn btn-xs u-btn-outline-aqua g-mr-10">评论</a>
                                    </li>
                                    ...
                                    <li class="list-inline-item">
                                        <i class="icon-clock"></i>{{ $moment(post.timestamp).format('YYYY年MM月DD日HH:mm:ss')}}
                                    </li>
                                    <li class="list-inline-item g-mx-10">/</li>
                            ...
                            <div id="like-post" class="row">
                                <div class="col-lg-3">
                                    <button @click="onLikeOrUnlikePost(post)" :class="btnOutlineColor"
                                            class="btn btn-block g-rounded-50 g-py-12 g-mb-10">
                                        <i class="icon-heart g-pos-rel g-top-1 g-mr-5"></i> 喜欢<span
                                            v-if="post.likers && post.likers.length > 0"> | {{ post.likers.length }}</span>
                                    </button>
                                </div>
                                <div class="col-lg-9">
                                    <ul v-if="post.likers" class="list-inline mb-0">
                                        <li class="list-inline-item"
                                            v-for="(liker, index) in post.likers" :key="index">
                                            <router-link
                                                    :to="{ path: `/user/${liker.id}` }"
                                                    :title="liker.name || liker.username">
                                                <img class="g-brd-around g-brd-gray-light-v3 g-pa-2 g-width-40 g-height-40 rounded-circle rounded mCS_img_loaded g-mt-3"
                                                     :src="liker.avatar"
                                                     :alt="liker.name || liker.username">
                                            </router-link>
                                        </li>
                                    </ul>
                                </div>
                ...
                <i class="icon-options-vertical g-pos-rel g-top-1"></i>
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right rounded-0 g-mt-10">
                                        ...
                                        <router-link :to="{ path: $route.path, query: { page: 1, per_page: 20 }}"
                                                     class="dropdown-item g-px-10">
                                            <i class="icon-fire g-font-size-12 g-color-gray-dark-v5 g-mr-5"></i> 每页 20
                                            条顶层评论
                                        </router-link>
                            ...
                            <!-- Add Comment Form -->
                            <form id="addCommentForm" v-if="sharedState.is_authenticated"
                                  @submit.prevent="onSubmitAddComment" @reset.prevent="onResetAddComment"
                                  class="g-mb-40">
                                <div class="form-group">
                                    <textarea v-model="commentForm.body" class="form-control" id="commentFormBody"
                                              rows="5" placeholder=" 写下你的评论 ..."></textarea>
                                    <small class="form-control-feedback" v-show="commentForm.bodyError">{{
                                        commentForm.bodyError }}
                                    </small>
                                </div>
                                <button type="reset" class="btn btn-secondary">Cancel</button>
                                ...
                                <div v-for="(comment, index) in comments.results" :key="index">
                                    ...
                                    <div :id="'c' + comment.id"
                                        ...
                                        <router-link :to="{ path: `/user/${comment.author.id}/` }">
                                            ...
                                            <img class="d-flex g-width-50 g-height-50 rounded-circle g-brd-around g-brd-gray-light-v4 g-pa-2 g-mt-3 g-mr-15"
                                        ...
                                        <div class="media-body">
                                            <div class="g-mb-15">
                                                <h5 v-if="comment.author.id === comment.post.author"
                                                    class="h5 g-color-gray-dark-v1 mb-0">
                                                    <router-link :to="{ path: `/user/${comment.author.id}/` }"
                                                                 class="comment-author g-text-underline--none--hover">{{
                                                        comment.author.name || comment.author.username }}
                                                    </router-link>
                                                    <button class="btn btn-xs u-btn-inset u-btn-outline-red g-mr-5">
                                                        博文作者
                                                    </button>
                                                </h5>
                                                <h5 v-else class="h5 g-color-gray-dark-v1 mb-0">
                                                    <router-link :to="{ path: `/user/${comment.author.id}/` }"
                                                                 class="comment-author g-text-underline--none--hover">{{
                                                        comment.author.name || comment.author.username }}
                                                    </router-link>
                                            ...
                                            <div v-if="comment.disabled" class="g-color-red g-mb-15">此评论包含不良信息，已被禁止显示.
                                            </div>
                                            <div v-else>
                                                <!-- vue-markdown 开始解析markdown，它是子组件，通过 props 给它传值即可
                                            ...
                                            <ul class="list-inline d-sm-flex my-0">
                                                <li v-if="!comment.disabled" class="list-inline-item g-mr-20">
                                                    <a @click="onLikeOrUnlike(comment)"
                                                       class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover"
                                                       href="javascript:;">
                                                        <i :class="{ 'g-color-red': comment.liked.indexOf(parseInt(sharedState.user_id)) !== -1 }"
                                                           class="icon-like g-pos-rel g-top-1 g-mr-3"></i>
                                                        <span v-if="comment.liked.length > 0"> {{ comment.liked.length }} 人赞</span>
                                                        <span v-else>赞</span>
                                                    </a>
                                                </li>
                                                <li v-if="!comment.disabled" class="list-inline-item g-mr-20">
                                                    <a @click="onClickReply(comment)"
                                                       class="comment-reply-link u-link-v5 g-color-gray-dark-v4 g-color-primary--hover"
                                                       href="javascript:;">
                                                ...
                                                <ul class="list-inline mb-0 ml-auto">
                                                    <li style="display: none;" class="list-inline-item g-mr-5">
                                                        <button @click="onEditComment(comment)"
                                                                ...
                                                                data-toggle="modal" data-target="#editCommentModal">编辑
                                                        </button>
                                                    </li>
                                                    <li v-if="!comment.disabled && post.author.id === sharedState.user_id"
                                                        class="list-inline-item">
                                                        <button @click="onDisabledComment(comment)"
                                                                class="btn btn-xs u-btn-outline-purple">屏蔽
                                                        </button>
                                                    </li>
                                                    <li v-if="comment.disabled && post.author.id === sharedState.user_id"
                                                        class="list-inline-item">
                                                        <button @click="onEnabledComment(comment)"
                                                                class="btn btn-xs u-btn-outline-aqua">恢复
                                                        </button>
                                                    </li>
                                                    <li v-if="comment.author.id === sharedState.user_id || post.author.id === sharedState.user_id"
                                                        class="list-inline-item">
                                                        <button @click="onDeleteComment(comment)"
                                                                class="btn btn-xs u-btn-outline-red">删除
                                                        </button>
                                                    </li>
                                                </ul>
                                            </ul>
                                    ...
                                    <div class="g-ml-40 comment-item media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20"
                                         v-if="comment.child"
                                         v-for="(child, cindex) in comment.child" :key="cindex"
                                         :id="'c' + child.id">
                                        ...
                                        <div class="media-body">
                                            <div class="g-mb-15">
                                                <h5 v-if="child.author.id === child.post.author"
                                                    class="h5 g-color-gray-dark-v1 mb-0">
                                                    <router-link :to="{ path: `/user/${child.author.id}/` }"
                                                                 class="comment-author g-text-underline--none--hover">{{
                                            ...
                                            <div v-if="child.disabled" class="g-color-red g-mb-15">此评论包含不良信息，已被禁止显示.
                                            </div>
                                            <div v-else>
                                                <!-- vue-markdown 开始解析markdown，它是子组件，通过 props 给它传值即可
                                                v-highlight 是自定义指令，用 highlight.js 语法高亮 -->
                                            ...
                                            <ul class="list-inline d-sm-flex my-0">
                                                <li v-if="!child.disabled" class="list-inline-item g-mr-20">
                                                    <a @click="onLikeOrUnlike(child)"
                                                       class="u-link-v5 g-color-gray-dark-v4 g-color-primary--hover"
                                                       href="javascript:;">
                                                        <i :class="{ 'g-color-red': child.liked.indexOf(parseInt(sharedState.user_id)) !== -1 }"
                                                           class="icon-like g-pos-rel g-top-1 g-mr-3"></i>
                                                ...
                                                <ul class="list-inline mb-0 ml-auto">
                                                    <li style="display: none;" class="list-inline-item g-mr-5">
                                                        <button @click="onEditComment(child)"
                                                                class="btn btn-xs u-btn-outline-purple"
                                                                data-toggle="modal" data-target="#editCommentModal">编辑
                                                        </button>
                                                    </li>
                                                    <li v-if="!child.disabled && post.author.id === sharedState.user_id"
                                                        class="list-inline-item">
                                                        <button @click="onDisabledComment(child)"
                                                                class="btn btn-xs u-btn-outline-purple">屏蔽
                                                        </button>
                                                    </li>
                                                    <li v-if="child.disabled && post.author.id === sharedState.user_id"
                                                        class="list-inline-item">
                                                        <button @click="onEnabledComment(child)"
                                                                class="btn btn-xs u-btn-outline-aqua">恢复
                                                        </button>
                                                    </li>
                                                    <li v-if="child.author.id === sharedState.user_id || post.author.id === sharedState.user_id"
                                                        class="list-inline-item">
                                                        <button
                                                                @click="onDeleteComment(child)"
                                                                class="btn btn-xs u-btn-outline-red"
                                                        >删除
                                                        </button>
                                                    </li>
                                                </ul>
                    ...
                    <div class="col-lg-3 g-pt-80">
                        ...
                        <div id="sticker" class="g-mb-50">
                            <div id="tocHeader" class="u-heading-v3-1 g-mb-15">
                                <h2 class="h5 u-heading-v3__title g-color-primary text-uppercase g-brd-primary">
                                    文章目录</h2>
                            </div>
